...
/*-----------------------------------------------------------------------**/
int Image::initialize (int nr, int nc)   /* nr = number of rows; nc = number of columns; */
{
...
/*-------------------------------------------------------------------*/
int Image::save (char file[100])   /* file[100]: file name of the image to be saved */
{
  FILE *outfile1;
  int i, j, Flag;
  ...
  else { // if gray level is larger than 255, set it to 255;
     for (i=0; i < NR; i++) {
      for (j=0; j < NC; j++) {
	putc((unsigned char) (((*(IM_r+i*NC+j))>255)?255:(*(IM_r+i*NC+j))), outfile1) ;
...
/*-----------------------------------------------------------------------**/
int Image::getint(FILE *fp)    /* fp: a point of the image file to be opened */
{
  int item, i, flag;
...
/*-------------------------------------------------------------------*/
int Image::read (char file[100])      // file[100]: name of the image to be opened; 
{
  FILE *fp;
  char str[50];
  unsigned char r, g, b;
